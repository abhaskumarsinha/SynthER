...
class Box:
    def __init__(self, low, high, shape, dtype):
        ...
        # if low/high are scalars, broadcast them to arrays
        if np.isscalar(low):
...
class DummyEnv:
    ...
    def get_normalized_score(self, score):
        print(f"[dummy d4rl] get_normalized_score({score})")
        # Just return a mocked normalized value (usually between 0 and 1)
        return score / 1.0
...
# 2c. TransformObservation
transform_mod = types.ModuleType("gym.wrappers.transform_observation")
class TransformObservation:
    def __init__(self, env, fn):
        # Important: store these under names Python won’t proxy
    ...
    def step(self, action):
        ...
        return self.fn(obs), reward, done, info

    def __getattr__(self, name):
        # forward anything else (like action_space, observation_space, render, etc.)
        return getattr(self.env, name)

# register it just like you did flatten_observation
transform_mod.TransformObservation = TransformObservation
sys.modules["gym.wrappers.transform_observation"] = transform_mod
...
class Actor(nn.Module):
    ...
    def act(self, state: np.ndarray, device: str = "cpu") -> np.ndarray:
        state = torch.tensor(state.reshape(1, -1), device='cpu', dtype=torch.float32)
...
def train(config: TrainConfig):
    ...
    critic_1 = Critic(
        state_dim, action_dim, hidden_dim=config.network_width, n_hidden=config.network_depth).to('cpu')
